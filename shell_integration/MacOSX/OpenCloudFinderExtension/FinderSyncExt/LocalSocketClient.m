 ...
 * https://github.com/nextcloud/desktop/blob/master/shell_integration/MacOSX/NextcloudIntegration/NCDesktopClientSocketKit/LocalSocketClient.m
 */
...
{
    NSLog(@"LocalSocketClient: Initiating with socket path %@", socketPath);
    self = [super init];
...
- (void)start
{
    ...
    if(socketPathByteCount > maxByteCount) {
        NSLog(@"LocalSocketClient: Socket path '%@' is too long: maximum %i, got %lu", _socketPath, maxByteCount, socketPathByteCount);
        return;
    }
    
    NSLog(@"LocalSocketClient: Opening socket...");
    ...
    NSLog(@"LocalSocketClient: Socket opened. Connecting to '%@'...", _socketPath);
    ...
    if(fcntl(_sock, F_SETFL, flags | O_NONBLOCK) == -1) {
        NSLog(@"LocalSocketClient: Could not set socket to non-blocking mode: '%@'", [self strErr]);
        [self restart];
        return;
    }
...
- (void)sendMessage:(NSString *)message
{
    dispatch_async(_localSocketQueue, ^(void) {
        ...
        if(writeSourceIsSuspended) {
            NSLog(@"LocalSocketClient: Resuming write dispatch source.");
...
- (void)writeToSocket
{
    ...
    if([_outBuffer length] == 0) {
        NSLog(@"LocalSocketClient: Empty out buffer, suspending write dispatch source.");
    ...
    if(bytesWritten == 0) {
        ...
        if([_outBuffer length] == 0) {
            NSLog(@"LocalSocketClient: Out buffer emptied, suspending write dispatch source.");
            dispatch_suspend(_writeSource);
        }
    }
}
